        ...
        static string[] ArrangeWords(string strVertWord, string[] strHorWords)
        {
            ...
            while (running)
            {
                ...
                if (!flag)
                {
                    for (int i = 0; i < strUsedWords.Length; i++)
                    {
                        //Перебирается каждая пустая позиция в массиве
                        if (strUsedWords[i] == null)
                        {
                            running = true;
                            //Для буквы, соответствующей пустой позиции выбирается первое попавшееся слово (см. пояснение к (3*))
                            for (int j = 0; j < strHorWords.Length; j++)
                            {
                        ...
                        //Если одно слово было подставлено, попробовать пробежаться по циклу while ещё раз
                        if (flag) break;
                    }
                }
